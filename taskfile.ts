    ...
    template: [
        {
            ...
            body: <ShellActivityOption>{
                ...
                shell: (ctx: NodeActivityContext) => {
                    let packages = ctx.platform.getFolders('packages').filter(f => /(model|mvc|typeorm-adapter)$/.test(f));
        ...
        },
        {
            ...
            body: [
                ...
                },
                {
                    ...
                    each: (ctx: NodeActivityContext) => ctx.platform.getFolders('packages').filter(f => /(model|mvc|typeorm-adapter)$/.test(f)),
                    body: {
